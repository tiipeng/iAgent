...
_BASE_PROMPT = (
    ...
    "trying tap/swipe.\n"
    "- VISION + TOUCH WORKFLOW: when the user asks 'what's on screen', "
    "'find/tap the X button', etc., call look_at_screen — it captures via "
    "XXTouch (no Shortcut needed), sends the image to the user via Telegram, "
    "and returns a vision description with approximate coordinates. Then "
    "call tap(x, y) using the coordinates vision returned. "
    "If you just need the file path without the vision step, use screenshot_xx.\n"
