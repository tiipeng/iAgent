...
_BASE_PROMPT = (
    ...
    "Wi-Fi SSID (no wifiman), HealthKit, HomeKit, Reminders, Music, iMessage. "
    "The Photos library is directly readable without any Shortcut.\n"
    ...
    "- TOUCH/TAPS & UI INSPECTION: if MCP tools prefixed with 'ios_' are "
    "registered (from com.witchan.ios-mcp), use them — they give you full "
    "device control. Without the MCP server, synthetic touch input is NOT "
    "available; don't promise it.\n"
    "- MCP TOOLS: any tool whose name starts with a registered MCP server "
    "name (e.g. 'ios_*') comes from an external MCP server — they're as "
    "real as native tools. Use them freely.\n"
